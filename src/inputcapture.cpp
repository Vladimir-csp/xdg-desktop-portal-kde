    ...
    connect(session, &InputCaptureSession::activated, this, [this, session](uint activationId, const QPointF &cursorPosition) {
        ...
        if (const QString appName = Utils::applicationName(session->appId()); !appName.isEmpty()) {
            notification->setText(
                xi18nc("@info %1 is the name of the application", "%1 is capturing input. Press <shortcut>Ctrl+Shift+Escape</shortcut> to disable.", appName));
        } else {
            notification->setText(xi18nc("@info", "An application is capturing input. Press <shortcut>Ctrl+Shift+Escape</shortcut> to disable."));
        }
